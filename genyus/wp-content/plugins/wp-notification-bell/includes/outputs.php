    ...
    if ( count( $data ) > 0 ) {
        ...
        foreach ( $data as $single_notification_id ) {
            ...
            foreach ( $trigger_array as $trigger_notification ) {
                ...
                if ( strtotime( $post_date ) < $trigger_date ) {
...
)
{
    ...
    if ( array_key_exists( 'comment_id', $trigger_notification ) || array_key_exists( 'count', $trigger_notification ) ) {
        ...
        if ( array_key_exists( 'comment_id', $trigger_notification ) ) {
            ...
            if ( array_key_exists( 'type', $trigger_notification ) && $trigger_notification['type'] === 'cfa' ) {
                $trigger_text = sprintf( __( '%s commented on your post.', 'wp-notification-bell' ), $trigger_notification['commenter'] );
            } else {
                $trigger_text = sprintf( __( '%s replied to your comment.', 'wp-notification-bell' ), $trigger_notification['commenter'] );
            }
    ...
    %2$s = Topic title in which user leave a comment', 'wp-notification-bell' ), $trigger_notification['reply_author_name'], $trigger_notification['topic_title'] );
    ...
    if ( strlen( $trigger_text ) > 0 ) {
        ...
        $trigger_output .= ' >';
        $trigger_output .= '<div>';
        $trigger_output .= ( strlen( $trigger_default ) > 0 ? $trigger_default : $trigger_text );
        $trigger_output .= '</div></a>';
        $trigger_output .= '</div>';
    }
